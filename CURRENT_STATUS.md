...
| --- | --- |
| Date | 2026-09-07 |
| Commit | `8644a94` (clean checkout; built and measured from a detached worktree at that commit) |
...
| Gate | Command | Result |
| --- | --- | ---: |
| Workspace tests | `cargo nextest run --workspace` | **1846 / 1846 passed** |
| Oracle harness tests | `pnpm run oracle:test` | **23 / 23 passed** |
| Oracle preset sweep — normal gate | `pnpm run oracle:sweep -- --all --maxDiagnostics 200` | **139 / 139 passed** |
| Oracle preset sweep — `--strictMessages` | same + `--strictMessages` | **139 / 139 passed** |
| Oracle preset sweep — `--strictSpans` | same + `--strictSpans` | **139 / 139 passed** |
| Oracle preset sweep — both strict flags | same + both | **139 / 139 passed** |
...
parity against the upstream TypeScript compiler. Across all 139 presets the
sweep saw 228 `tsc` diagnostics and 228 `surge-ts` diagnostics, with
...
- 139 oracle presets under `tests/compat-projects/`, all green at the normal
  gate **and at both strict gates** (see the table above).
- 363 compat-project fixtures in total; the ones not registered as oracle
...
| **trpc** | `dfbafa8` | 1244 | 1141 | measured baseline, **not** a parity target |
...
- **trpc** is a *measured baseline*, never a parity claim. `tsc` itself reports
  ...
  file/code/line drift is **143**: 123 diagnostics `tsc` reports and surge does
  not (led by TS7006 ×38, TS2339 ×29, TS2883 ×10, TS18048 ×8) and 20 surge-only
  (TS2322 ×4, TS4111 ×3, TS2339 ×3, TS2345 ×2, and eight single codes). On the
  1,121 locations both compilers agree on, message text matches **1121 / 1121**.
...
Re-measured at this snapshot's commit against `79cbcfc`, the last commit before
the second false-positive burn-down, four interleaved pairs: peak footprint
1.013 GB -> **1.012 GB** and wall median 4.39 s -> **4.46 s** on tRPC, 0.516 GB
-> **0.510 GB** and 3.40 s -> **3.39 s** on zod. Both neutral. (The absolute wall
figures are lower than the table above because the perf work committed between
those two commits landed in between; the table's columns are the ones that
bracket the regression.)
...
- **No general message-text or span/column parity claim.** Both strict sweeps
  are green across the 139 registered presets at this commit, which is a
...
- **No claim that trpc matches `tsc`.** It is a workload and a measured
  baseline; 143 diagnostics still differ in each direction combined.
