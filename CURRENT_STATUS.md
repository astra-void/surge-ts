...
| --- | --- |
| Date | 2026-09-07 |
| Commit | `5689e39` (clean checkout; built and measured from a detached worktree at that commit) |
...
| Gate | Command | Result |
| --- | --- | ---: |
| Workspace tests | `cargo nextest run --workspace` | **1859 / 1859 passed** |
| Oracle harness tests | `pnpm run oracle:test` | **23 / 23 passed** |
| Oracle preset sweep — normal gate | `pnpm run oracle:sweep -- --all --maxDiagnostics 200` | **147 / 147 passed** |
| Oracle preset sweep — `--strictMessages` | same + `--strictMessages` | **147 / 147 passed** |
| Oracle preset sweep — `--strictSpans` | same + `--strictSpans` | **147 / 147 passed** |
| Oracle preset sweep — both strict flags | same + both | **147 / 147 passed** |
...
parity against the upstream TypeScript compiler. Across all 147 presets the
sweep saw 241 `tsc` diagnostics and 241 `surge-ts` diagnostics, with
...
- 147 oracle presets under `tests/compat-projects/`, all green at the normal
  gate **and at both strict gates** (see the table above).
- 371 compat-project fixtures in total; the ones not registered as oracle
...
| **trpc** | `dfbafa8` | 1244 | 1134 | measured baseline, **not** a parity target |
...
- **trpc** is a *measured baseline*, never a parity claim. `tsc` itself reports
  ...
  file/code/line drift is **136**: 123 diagnostics `tsc` reports and surge does
  not (led by TS7006 ×38, TS2339 ×29, TS2883 ×10, TS18048 ×8) and 13 surge-only
  (TS4111 ×3, TS2339 ×2, TS2322 ×2, and six single codes). On the 1,121
...
GB and 3.35 -> 3.33 s on zod. And this snapshot's commit against `8905e69`,
which added JSON-module loading to the import graph: 1.086 -> 1.086 GB and
5.05 -> 5.10 s on tRPC, 0.547 -> 0.548 GB and 3.80 -> 3.80 s on zod. (Absolute
wall figures move between rounds with machine load; only the interleaved pairs
within a round compare.)
...
- **No general message-text or span/column parity claim.** Both strict sweeps
  are green across the 147 registered presets at this commit, which is a
...
- **No claim that trpc matches `tsc`.** It is a workload and a measured
  baseline; 136 diagnostics still differ in each direction combined.
