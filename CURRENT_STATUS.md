...
| `SURGE_PROMISE_NOMINAL=1` | Keep the lib's `Promise<T>` / `PromiseLike<T>` as the interface it is instead of collapsing it to its awaited `T` (a model that dates from when `await` was erased at parse time). With it `Promise<T> \| undefined` stops reading as `T \| undefined`, a promise used as its value is reported (`p.session` on a `Promise<Ctx>`), async bodies are typed against the awaited return type, and an async arrow's inferred type is `() => Promise<X>`. | Measured 2026-09-22 on branch `feat/null-type`, binary built from the tree recorded by commit `08d9061e` (seven corpora, gate on vs off): zustand −1 (a false TS2358 closed), trpc +1, tanstack-query +2/−1, every other corpus byte-identical; the oracle presets pass identically with the gate on and off (402 of 404, the same two `grammar-*` failures). What is still open: trpc `localLink.ts` reads `Awaited<V>` (the race's value through a generic helper is not evaluated), and two vitest `Mock<T>` shapes fail against `QueryFunction` / `MutationFunction` targets. Gate off is byte-identical on every sweep, corpus and oracle preset. |
