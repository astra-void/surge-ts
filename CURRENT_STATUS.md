...
| --- | --- |
| Date | 2026-09-07 |
| Commit | `8905e69` (clean checkout; built and measured from a detached worktree at that commit) |
...
| Oracle preset sweep — normal gate | `pnpm run oracle:sweep -- --all --maxDiagnostics 200` | **145 / 145 passed** |
| Oracle preset sweep — `--strictMessages` | same + `--strictMessages` | **145 / 145 passed** |
| Oracle preset sweep — `--strictSpans` | same + `--strictSpans` | **145 / 145 passed** |
| Oracle preset sweep — both strict flags | same + both | **145 / 145 passed** |
...
parity against the upstream TypeScript compiler. Across all 145 presets the
sweep saw 236 `tsc` diagnostics and 236 `surge-ts` diagnostics, with
...
- 145 oracle presets under `tests/compat-projects/`, all green at the normal
  gate **and at both strict gates** (see the table above).
- 369 compat-project fixtures in total; the ones not registered as oracle
...
| **trpc** | `dfbafa8` | 1244 | 1135 | measured baseline, **not** a parity target |
...
- **trpc** is a *measured baseline*, never a parity claim. `tsc` itself reports
  ...
  file/code/line drift is **137**: 123 diagnostics `tsc` reports and surge does
  not (led by TS7006 ×38, TS2339 ×29, TS2883 ×10, TS18048 ×8) and 14 surge-only
  (TS4111 ×3, TS2339 ×2, TS2322 ×2, and seven single codes). On the 1,121
...
GB and 3.35 -> 3.33 s on zod. (Absolute wall figures move between rounds with
machine load; only the interleaved pairs within a round compare.)
...
- **No general message-text or span/column parity claim.** Both strict sweeps
  are green across the 145 registered presets at this commit, which is a
...
- **No claim that trpc matches `tsc`.** It is a workload and a measured
  baseline; 137 diagnostics still differ in each direction combined.
