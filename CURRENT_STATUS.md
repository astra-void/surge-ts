...
| `SURGE_PROMISE_NOMINAL=1` | Keep the lib's `Promise<T>` / `PromiseLike<T>` as the interface it is instead of collapsing it to its awaited `T` (a model that dates from when `await` was erased at parse time). With it `Promise<T> \| undefined` stops reading as `T \| undefined`, a promise used as its value is reported (`p.session` on a `Promise<Ctx>`), async bodies are typed against the awaited return type, and an async arrow's inferred type is `() => Promise<X>`. | Measured 2026-09-22 on branch `feat/null-type` (binary built from the tree of the commit that adds this row; seven corpora, gate on vs off): zod +2, trpc +6, tanstack-query +20, zustand −1, others byte-identical; three oracle presets regress (`promise-relation-basic`, `function-expression-missing-return-basic`, `implicit-returns-any-return-basic`). Known causes, none fixed yet: `then`'s uninferred `TResult2 = never` default is not applied on the member path, `TResult1 \| PromiseLike<TResult1>` does not unwrap a returned promise, `Promise.resolve` / `Promise.all` still answer the awaited value, and a return or a string argument against an interface with a generic method is not checked (a gap the gate inherits, not one it causes). Gate off is byte-identical on every sweep, corpus and oracle preset. |
