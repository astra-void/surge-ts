...
| [REAL_PROJECT_COMPAT-HISTORY.md](REAL_PROJECT_COMPAT-HISTORY.md) | The `v0.60`–`v1.2.5` milestone log (including the auth-kit optimization series), the `v0.84` real-project audit, the per-feature `v0.7x`/`v0.8x` notes, and the superseded "current baseline" support lists formerly in `REAL_PROJECT_COMPAT.md`. |
