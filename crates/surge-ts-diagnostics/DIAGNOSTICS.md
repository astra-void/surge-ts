...
The coverage tests (`cargo test -p surge-ts-diagnostics --test coverage`) ensure that:
...
2. Run `cargo run -p surge-ts-diagnostics-codegen`.
3. Use the generated `Diagnostic::tsXXXX(...)` or `Diagnostic::surge_* (...)` accessor.
...
- `surge::*` codes are reserved for implementation-specific diagnostics.
