...
fn validate_code_policy(entry: &CatalogEntry) -> Result<()> {
    match entry.source {
        ...
        DiagnosticSource::TypescriptRust => {
            if !entry.code.starts_with("surge::") {
                return Err(validation_error(format!(
                    "custom diagnostic code must start with surge::: {}",
                    entry.code
                )));
            }
...
/// The number of `{N}` placeholders. Like tsc's `formatStringFromArgs`, only
/// a brace pair around digits is a placeholder; any other brace is message
/// text (TS1144's `'{' or ';' expected.`, TS1381's `` `{'}'}` ``).
fn placeholder_arity(message: &str) -> Result<usize> {
    ...
    while let Some(open) = rest.find('{') {
        ...
        if digits > 0 && after.as_bytes().get(digits) == Some(&b'}') {
            let index = after[..digits].parse::<usize>().map_err(|error| {
...
pub fn diagnostic_function_name(entry: &CatalogEntry) -> String {
    match entry.source {
        ...
        DiagnosticSource::TypescriptRust => {
            format!("surge_{}", entry.code["surge::".len()..].replace('-', "_"))
        }
    }
}
...
fn source_name(source: DiagnosticSource) -> &'static str {
    match source {
        ...
        DiagnosticSource::TypescriptRust => "surge-ts",
    }
}
