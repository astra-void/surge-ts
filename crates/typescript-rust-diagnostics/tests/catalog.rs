...
use typescript_rust_diagnostics_codegen::{
...
fn catalog_json_custom_codes_follow_policy() {
    for entry in catalog_entries() {
        if matches!(entry.source, CatalogSource::TypescriptRust) {
            assert!(
                entry.code.starts_with("typescript-rust::"),
...
fn generated_custom_diagnostic_formats_message() {
    let diagnostic = Diagnostic::typescript_rust_unsupported_module_syntax("example.ts");
    assert_eq!(
