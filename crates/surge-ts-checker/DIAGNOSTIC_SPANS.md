...
| surge::duplicate-type-parameter | duplicate type-parameter name span |
...
- **Unsupported Declaration Syntax**: `surge::unsupported-declaration` points to the syntax token in loaded declaration files.
- **Ambient Module Fallback**: Missing exports point to the import specifier.
- These custom `surge::*` diagnostics are catalog entries like the `TSxxxx` codes, but their spans are still decided at the callsite.
