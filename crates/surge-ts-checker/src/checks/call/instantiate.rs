    ...
    match parameter_type {
        ...
        ParsedType::Union(expected_types) => {
            ...
            let mixes_naked_and_structured = expected_types
                .iter()
                .any(|member| is_naked_type_parameter(member, substitution))
                && expected_types
                    .iter()
                    .any(|member| !is_naked_type_parameter(member, substitution));
            ...
            // Nothing matched a structured member is still an answer for the
            // naked one: `eq(a, b: T | undefined)` binds `T` from `b` even
            // though `b` matched neither `undefined` nor anything else. Bailing
            // here dropped every candidate but the first argument's.
