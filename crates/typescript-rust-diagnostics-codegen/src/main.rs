...
fn main() -> ExitCode {
    ...
    let diagnostics_dir = crates_dir.join("typescript-rust-diagnostics");
    ...
    let entries = match typescript_rust_diagnostics_codegen::load_catalog(&catalog_path) {
        Ok(entries) => entries,
        Err(error) => {
    ...
    let generated = match typescript_rust_diagnostics_codegen::generate_rust(&entries) {
        Ok(output) => output,
        Err(error) => {
    ...
    let snapshot = match typescript_rust_diagnostics_codegen::generate_snapshot_toml(&entries) {
        Ok(output) => output,
        Err(error) => {
