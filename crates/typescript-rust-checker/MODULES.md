...
- `lib.d.ts` loading and `@types` discovery remain unsupported. v0.72/v0.72.1 uses synthetic built-ins (e.g. `console`, `Promise`, `Math`), not physical `lib.d.ts`. `Array<T>` and `ReadonlyArray<T>` are modeled enough to preserve element diagnostics, while utility types mostly suppress TS2304 without mapped/conditional semantics. `noLib: true` disables these synthetic built-ins. DOM, Node, `@types`, and true lib loading remain unsupported.
